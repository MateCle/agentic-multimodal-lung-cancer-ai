...
def run_baseline(cohort: str, split_file: str) -> dict:
    ...
    print(
        f"  Event rate  — Train: {event_rate_train:.2%} | Val: {event_rate_val:.2%} | Test: {event_rate_test:.2%}"
    )
    ...
    print(
        f"  PCA components: {n_components} (explained variance: {pca.explained_variance_ratio_.sum():.2%})"
    )
    ...
    # --- CoxPH Model ---
    cph = CoxPHBaseline()
    cph.fit(X_train_pca, y_train)

    print("\n  [Overall Performance]")
    ci_train = cph.score(X_train_pca, y_train)
    ci_val = cph.score(X_val_pca, y_val)
    ci_test = cph.score(X_test_pca, y_test)
    ...
    completeness_val = c_index_by_completeness(is_complete_val, X_val_pca, y_val, cph)

    print("\n  [Breakdown by Modality Completeness - Test Set]")
    completeness_test = c_index_by_completeness(
        is_complete_test, X_test_pca, y_test, cph
    )
...
if __name__ == "__main__":
    ...
    results.append(
        run_baseline(
            cohort="luad", split_file="tcga_luad_DSS_k3_r1_test0.2_val0.2_seed42.json"
        )
    )
    results.append(
        run_baseline(
            cohort="lusc", split_file="tcga_lusc_DSS_k5_r1_test0.2_val0.2_seed42.json"
        )
    )
    ...
    for r in results:
        ...
        print(
            f"  {r['cohort']}: C-index={r['ci_test']:.4f} (Comp: {ci_comp} / Incomp: {ci_incomp})"
        )
